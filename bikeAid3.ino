...
void setup() {
    ...
    // based on https://docs.particle.io/tutorials/integrations/google-maps/
    locator.withSubscribe(locationCallback).withLocatePeriodic(30);
    Particle.subscribe("hook-response/locateMe", cityHandler, MY_DEVICES);
    ...
    snprintf(lastCheckedCity, sizeof(lastCheckedCity), "Arhus");
...
void cityHandler(const char *event, const char *data) {
    // Handle the response from google maps api with city name
    snprintf(lastCheckedCity, sizeof(data), "%s", data);

    Serial.printf("city received = %s ",lastCheckedCity);
    Serial.println();
...
void locationCallback(float lat, float lon, float accuracy) {
  ...
  // - Accuracy of estimated location (in meters)
    ...
    char data[256];
    snprintf(data, sizeof(data), "{\"latlng\":\"%f,%f\"}", lat, lon);
   
    // publishing locateMe event with current location in order to get the city we're in (from Google Maps API)
    Particle.publish("locateMe", data, PRIVATE);
...
void publishHumidityReq(){
    ...
    if(publishCounter == 0){
        ...
        char data[256];
        snprintf(data, sizeof(data), "{\"city\":\"%s\"}", lastCheckedCity);
